...
async def hi(message: Message, bot: Bot):
    ...
    text = message.text or message.caption
    if old_msg:
        new_msg = await create_message(message_id=message.message_id, text=text, user_id=message.from_user.id)
...
async def help(message: Message, bot: Bot):
    await message.answer(f"Нажми на этот символ --->[<code>⠀</code>], затем вставь его в ответ на самоуничтожающееся сообщение, чтобы скачать его" )
